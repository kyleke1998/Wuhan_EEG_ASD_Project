...
for j, (train_val_idx, test_idx) in enumerate(outer_cv_splits):
    ...
    print("new shape: ", X_train_val.shape)
...
# Initialize the array to store the nested scores
nested_sensitivity = np.zeros(50)
nested_specificity = np.zeros(50)
nested_roc_auc = np.zeros(50)
nested_f1_macro = np.zeros(50)
nested_accuracy = np.zeros(50)
nested_C = np.zeros(50)
...
# Loop over the outer loop splits
for j, (train_val_idx, test_idx) in enumerate(sss.split(X, y)):
